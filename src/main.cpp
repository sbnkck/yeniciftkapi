...
// ---- UART frame snapshot yapısı ----
typedef struct
{
   uint32_t t_ms;
   uint8_t len;
   uint8_t payload[SERIAL_SIZE]; // 50 byte
} DoorFrame_t;

// ---- TX update yapısı (tek byte güncelleme) ----
typedef struct
{
   uint8_t index;
   uint8_t value;
...
static inline void door_tx_set(uint8_t index, uint8_t value)
{
   if (!q_tx_update)
      return;
...
static inline bool door_ack_rx_peek(uint8_t out[SERIAL_SIZE])
{
   if (!q_ack_frame)
      return false;
   DoorFrame_t f;
   if (xQueuePeek(q_ack_frame, &f, 0) != pdTRUE)
      return false;
   memcpy(out, f.payload, SERIAL_SIZE);
   return true;
}
static inline bool door_rx_peek(uint8_t out[SERIAL_SIZE])
{
   if (!q_rx_frame)
      return false;
   DoorFrame_t f;
   if (xQueuePeek(q_rx_frame, &f, 0) != pdTRUE)
      return false;
   memcpy(out, f.payload, SERIAL_SIZE);
   return true;
}
static inline float step_sync_duty_factor(int16_t master_step,
                                          int16_t slave_step)
{
   int16_t diff;
   ...
   // Deadband
   if (abs(diff) <= STEP_SYNC_DEADBAND)
      return 1.0f;

   float norm = (float)diff / (float)STEP_SYNC_WINDOW; // -1 .. +1
   ...
   while (1)
   {
      ...
      if (acil_stop_flag == false)
      {
         ...
         if (baski_flag)
         {
            ...
            if (kapi_rutbesi == SLAVE && (adim_sayisi < (kilit_birakma_noktasi * adim_derece_carpani))) // kapanaırken kilit noktasndan sonrasına geldiğini servere saöyluyor
            {
               door_tx_set(client_kilit_index, 1);
            }
         }
      }
   }
...
void DutyHesaplama()
{
   if (rpm > 0)
      rpmTespit = 0;
   float gain_scale = 5;
   double Kf = 0.007; /* --- Mevcut parametre ayarların AYNEN KALIYOR --- */
   if ((hareket_sinyali == kapi_ac_sinyali))
   {
      if ((adim_sayisi < 100))
      {
   ...
   eski_hata = hata; /* --- RPM sürtünme kompanzasyonu --- */
   double rpm_error;
   if (hareket_sinyali == kapi_ac_sinyali)
      rpm_error = hedefRPMharitasi[adim_sayisi] - rpm;
   else
      rpm_error = hedefRPMharitasi_kapa[adim_sayisi] - rpm;
   double friction_comp = Kf * rpm_error; /* ---------- DUTY HESABI ---------- */
   duty += delta;
   duty += friction_comp; /* 🔴🔴 MASTER–SLAVE ADIM SENKRONU 🔴🔴 */
   float sync_factor = step_sync_duty_factor(ref_adim_sayisi, adim_sayisi);
   duty *= sync_factor; /* 🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴 */ /* --- Clamp --- */
   if (duty < min_duty)
      duty = min_duty;
   if (duty > max_duty)
      duty = max_duty; /* --- Filtre --- */
...
void kapi_ac_fonksiyonu()
{
   ...
   if (rpm == 0 && adim_sayisi < maksimum_kapi_boyu && tanima_hizi_flag == false && faulttan_kapata == false) // motor durduğunda tekrar ilk hareketin sağlanması için
   {
      ...
      while (hareket_sinyali != kapi_kapat_sinyali)
      {
         ...
         if (count > (kapama_baski_suresi / 10)) // 1.5 sn bekleyecek açılamazsa kapata geçecek
         {
            ...
            if (kapi_rutbesi == MASTER)
               door_ack_rx_peek(rx_local);
            REF_ADIM_CEK;
            if (ref_adim_sayisi > adim_sayisi)
               printf("diger kapi bekleniyor\n");
            if (kapi_rutbesi == SLAVE)
            {
               printf("slave master e   bekliyor\n");
               while (kapat_flag == false && (counter < 2000))
               {
                  vTaskDelay(10 / portTICK_RATE_MS);
                  counter++;
               }
            ...
            if (kapi_rutbesi == MASTER)
               door_ack_rx_peek(rx_local);
            REF_ADIM_CEK;
            while (((ref_adim_sayisi - (iki_kapi_aci_farki * adim_derece_carpani)) > adim_sayisi) && (counter < 2000))
            {
               if (kapi_rutbesi == SLAVE)
                  door_rx_peek(rx_local);
               if (kapi_rutbesi == MASTER)
               ...
               motor_surme(baski_duty);
               vTaskDelay(10 / portTICK_RATE_MS);
               printf("baski yedi hafif acmaya calisiyor\n");
            }
            printf("diger kapi geldi\n");
   ...
   //(fault_siniri>0 && digitalRead(ac_pini) == 0)) // açma - durdurma işlemi.
   {
      ...
      if (kapi_rutbesi == SLAVE)
      {
         door_tx_set(client_kilit_index, 0);
      }
      ...
      if (calisma_yontemi == otomatik_kapan) // otomatik
      {
         ...
         if (kapi_rutbesi == MASTER)
         {
            while (adim_sayisi >= (maksimum_kapi_boyu - 10))
            {
               ...
               if (zaman_timeout <= acik_kalma_suresi)
               {
                  ...
                  if (zaman_timeout > (acik_kalma_suresi - 20))
                  {
                     door_tx_set(client_kapa_index, 1);
                     Serial.println("Slave kapa komutu gonderildi.33");
                  }
               }
               else
               {
         ...
         if (kapi_rutbesi == SLAVE)
         {
            ...
            door_rx_peek(rx_local);
            REF_ADIM_CEK;
            printf("ref_adim_sayisi %d ,maksimum_kapi_boyu %.2f,iki_kapi_aci_farki %d\n",ref_adim_sayisi,maksimum_kapi_boyu,rx_local[client_iki_kapi_aci_farki_index]);
            while (ref_adim_sayisi > (maksimum_kapi_boyu - (rx_local[client_iki_kapi_aci_farki_index] * adim_derece_carpani) + 10))
            {
               // door_tx_set(client_kapa_index, 1);
               door_rx_peek(rx_local);
               REF_ADIM_CEK;
               Serial.println("Slave aciyi bekliyor.");
               vTaskDelay(10 / portTICK_RATE_MS);
            }
         }
...
void kapi_kapa_fonksiyonu()
{
   ...
   REF_ADIM_CEK;
   if ((kapi_rutbesi == SLAVE) &&
       (ref_adim_sayisi > (kilit_birakma_noktasi * adim_derece_carpani)) &&
       (adim_sayisi < (kilit_birakma_noktasi * adim_derece_carpani)) &&
       (iki_kapi_aci_farki != 0)) // slave klit bölgesine gelmediyse
   {
      motor_surme(bekleme_duty);
      do
      {
         ...
         door_rx_peek(rx_local);
         REF_ADIM_CEK;
         printf("kilit noktasında bekliyorum\n");
      } while ((ref_adim_sayisi >
                (kilit_birakma_noktasi * adim_derece_carpani)) &&
               ac_flag == false);
   }
   if (kapi_rutbesi == MASTER && (adim_sayisi < (kilit_birakma_noktasi * adim_derece_carpani))) // klit bölgesine geldiysen geldiğini söyle
   {
      door_tx_set(client_kilit_index, 1);
   }
...
void kilit_ac()
{
   ...
   if (adim_sayisi < (kilit_birakma_noktasi * adim_derece_carpani)) // sadece kapalıyken kilidi açbilmesi için bu flaga bakılıyor.
   {
...
void ble_data_guncelle()
{
   ...
   { //  BLE cihazı bağlı ise BLE datası gönderilecektir.
     ...
     // ble_sayaci = 0;
      ...
      ble_yollanan_dizi_global[107] = ble_alinan_deger * 2;
      ble_yollanan_dizi_global[108] = kapi_rutbesi;
...
void ble_data_al()
{
   ...
   if (sifre_1 == 121 and sifre_2 == 122)
   {
      ...
      case 105:
         ...
         break;
      case 107:
         iki_kapi_aci_farki = ble_alinan_deger / 2;
         Serial.print("BLE Al Fn: iki_kapi_aci_farki = ");
         Serial.println(iki_kapi_aci_farki * 2);
         break;
      case 108:
...
void eeprom_oku_fn()
{
   ...
   if (mod > 200)
   {
      ...
      EEPROM.write(79, mod);
      mod = EEPROM.read(79);
      printf("vrsyln mod_____________________: %d \n", mod);
   }
   else
   {
      printf("mod_____________________: %d \n", mod);
   }
   kapi_rutbesi = EEPROM.read(108);
   if (kapi_rutbesi > 200)
   {
      kapi_rutbesi = SLAVE; //  Varsayılan Değer  tek kanat
      EEPROM.write(108, kapi_rutbesi);
      kapi_rutbesi = EEPROM.read(108);
...
static void test_task(void *arg)
{
   ...
   while (1)
   {
      ...
      if (kapi_rutbesi == MASTER)
         door_ack_rx_peek(rx_local);
      REF_ADIM_CEK;
      if (test_flag == true && hareket_sinyali != kapi_ac_sinyali && adim_sayisi < 10 && kapi_rutbesi == MASTER && ref_adim_sayisi < 50)
      {
...
void role_state(void *arg)
{
   while (1)
   {
      ...
      else if ((kilit_state == 0 && (adim_sayisi > (kilit_birakma_noktasi * adim_derece_carpani))) || (((millis() - kilit_timeout) > 5000) && kilit_state == 0)) //
      {
         kilit_kapat();
...
void handle_ack_frame(uint8_t *payload, int len)
{
   ...
   if (q_ack_frame)
   {
      DoorFrame_t f{};
      f.t_ms = millis();
      f.len = (uint8_t)min(len, (int)SERIAL_SIZE);
      memcpy(f.payload, payload, f.len);
      xQueueOverwrite(q_ack_frame, &f);
   }

   // ACK geldi bilgisini ver
   uart_ack_geldi = true;
...
void handle_data_frame(uint8_t *payload, int len)
{
   ...
   if (q_rx_frame)
   {
      ...
      f.len = (uint8_t)min(len, (int)SERIAL_SIZE);
      memcpy(f.payload, payload, f.len);
      xQueueOverwrite(q_rx_frame, &f);
   }

   if (kapi_rutbesi == MASTER)
   {
      // Şu an MASTER tarafında DATA işlenmiyor
      return;
   }
   ...
   Max_RPM = double(rx_local[client_max_rpm_index]) * hiz_katsayisi;
   kapama_max_rpm = double(rx_local[client_kapama_max_rpm_index]) * hiz_katsayisi;
   mentese_yonu = !bool(rx_local[client_mentese_index]);
   acil_stop_client = rx_local[client_acil_stop_index];

   kapama_baski_suresi =
       double(rx_local[client_baski_suresi_index]) * kapama_baski_suresi_ks * 2;

   acik_kalma_suresi =
       double(rx_local[client_acik_kalma_suresi_index]) * 100;

   akim_siniri =
       double(rx_local[client_kuvvet_siniri_index]);

   push_run_flag =
       bool(rx_local[client_push_run_index]);

   kapama_baski_gucu =
       double(rx_local[client_baski_gucu_index]) * kapama_baski_gucu_ks;

   kapi_acma_derecesi =
       double(rx_local[client_derece_index]) * 2;

   REF_ADIM_CEK;

   iki_kapi_aci_farki=rx_local[client_acik_kalma_suresi_index];
}
...
void master_send_task(void *arg)
{
   ...
   while (1)
   {
      ...
      // ---- TX UPDATE QUEUE APPLY ----
      if (q_tx_update)
      {
         TxUpdate_t u;
         // queue'da biriken tüm güncellemeleri uygula
         while (xQueueReceive(q_tx_update, &u, 0) == pdTRUE)
         {
            if (u.index < SERIAL_SIZE)
      ...
      tx_data[client_iki_kapi_aci_farki_index] = iki_kapi_aci_farki & 0xFF;        //*
